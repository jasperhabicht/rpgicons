...
Since the \macro{l3draw} package is experimental and subject to ongoing updates, the \macro{dndicons-l3draw} package provides a fallback to set of established and more stable macro definitions (being available since the last major TeX Live release) which is active per default. Loading the \macro{dndicons-l3draw} package with the option \macro{edge} will use functions that are only available in the most recent version of the \macro{l3draw} package.
