...
\def\dndiconsfileversion{1.4.3}
\def\dndiconsfiledate{22 February 2024}
...
\NewExpandableDocumentCommand{\printdoctitle}{ o m o m m o m }{%
    \hypersetup{
        pdfauthor={#5},
    ...
    \Huge{\bfseries #2}\bigskip\par
    \Large #4\bigskip\par
    \large #5%
    \IfValueT{#6}{%
        \,\footnote{#6}\bigskip\par
    }%
    #7 \bigskip\par
    \rule{\textwidth}{.08em}%
    \endgroup
...
\usepackage[compat]{dndicons-l3draw}
...
\changes{v1.4.2}{2024/02/21}{Define alternative set of commands in \titlemacro{l3draw} variant.}
...
The option \macro{baseline} can be used to adjust the baseline of the icons. A larger value for the baseline will shift the icon downwards relative to the baseline of the surrounding text. The default value of the baseline is -3.5\,pt.
...
A variant of the package that uses the \macro{l3draw} package instead of the \macro{tikz} package is loaded by calling \macro{\usepackage{dndicons-l3draw}} in the preamble of the document. The \macro{l3draw} package is an experimental package that provides only basic drawing functionality and therefore loads quicker than Ti*k*Z. The \macro{l3draw} variant thus only supports a certain set of option for styling the icons. 

\begin{macrodef}compat\end{macrodef}
The \macro{dndicons-l3draw} package provides the option \macro{compat}. Since the \macro{l3draw} package is experimental and macro definitions can change, it can be useful not to use the most current macro definitions that may not yet be available in a local TeX installation. Loading the \macro{dndicons-l3draw} package with the option \macro{compat} ensures that only functions are used that are available in the TeX installation prior to the current one.
...
The \macro{color} option sets the color of strokes and fills in general while the \macro{stroke} option and the \macro{fill} option set the color only for strokes or fills respectively. The \macro{line width} sets the line width for strokes. Using the \macro{scale} and \macro{rotate} options, the shape can be scaled and rotated.
