...
\def\dndiconsfileversion{1.5.4}
\def\dndiconsfiledate{6 March 2024}
...
\changes{v1.5.3}{2024/03/03}{Optimsation of code in \titlemacro{l3draw} variant.}
...
accessible
\end{macrodef}
The \macro{accessible} option adds an alternative text to the relevant icon, making it accessible. It adds \macro{\ActualText} and \macro{\Alt} marks in the PDF which means that the shape of the relevant icon is read by screen readers and extracted when copied. It is necessary to call \macro{\DocumentMetadata{}} before loading a \macro{\documentclass}, preferably with tagging enabled. 
