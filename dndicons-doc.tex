...
\def\dndiconsfileversion{1.7.0}
\def\dndiconsfiledate{17 March 2024}
...
\section{L3 package variant}

A variant of the package that uses the \macro{l3draw} package instead of the \macro{tikz} package is loaded by calling \macro{\usepackage{dndicons-l3}} in the preamble of the document after having installed the file \macro{dndicons-l3.sty}. The \macro{l3draw} package is an experimental package that provides only basic drawing functionality and therefore loads quicker than Ti*k*Z. The L3 variant thus only supports a certain set of option for styling the icons. 

In contrast to the \macro{dndicons} package that uses Ti\emph{k}Z which in turn loads the \macro{xcolor} package, the \macro{dndicons-l3} package does not load the \macro{xcolor} package. Instead, it makes use of the \macro{l3color} module which uses as similar syntax like the \macro{xcolor} package, but color defintions made using the \macro{l3color} module are not directly usable via commands provided by the \macro{xcolor} package. Therefore, setting a color using the \macro{\color} macro provided by the \macro{xcolor} package won't affect the color of the icons.
...
|\DndiconsCondition|{<shape>}[<options>]
\end{macrodef}
If the \macro{dndicons} package is to be loaded together with the \macro{dndicons-l3} package or some other package that defines one of the above described basic commands \macro{\die}, \macro{\ability}, \macro{\saving}, \macro{\spell}, \macro{\spellschool}, \macro{\damage}, \macro{\attack} or \macro{\condition}, this set of alternative commands can be used. These commands are exact copies of the relevant basic commands and take the same set or arguments. 
