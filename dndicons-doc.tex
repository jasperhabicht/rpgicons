...
\lstdefinestyle{dndiconsdoc}{
    ...
    literate=*{<}{{{\color{gray}\guilsinglleft}}}1 {>}{{{\color{gray}\guilsinglright}}}1,
    language=simpletex,
}
...
\NewCommandCopy\tikzdie\die
\NewCommandCopy\tikzability\ability
\let\die\undefined
\let\ability\undefined

\usepackage{dndicons-l3draw}
\NewCommandCopy\liiidie\die
\NewCommandCopy\liiiability\ability
\RenewCommandCopy\die\tikzdie
\RenewCommandCopy\ability\tikzability
...
A variant of the package that uses the \titlemacro{l3draw} package instead of the \titlemacro{tikz} package is loaded by calling \macro{\usepackage{dndicons-l3draw}} in the preamble of the document. The \titlemacro{l3draw} package is an experimental package that provides only basic drawing functionality and therefore loads quicker than Ti*k*Z. The \titlemacro{l3draw} variant thus only supports a certain set of option for styling the icons. 
...
\dndiconsset
\end{macrodef}
Apart from setting the options to the commands directly, it is also possible to set them globally using the \macro{\dndiconsset} command. When the above described options for styling the icons are selected via \macro{\dndiconsset}, they need to be wrapped in the key \macro{style} as shown in the example below.

\RenewCommandCopy\ability\liiiability
\begin{codeexample}
\dndiconsset{
    style={
        color=blue
...
after sep
\end{macrodef}
Similar to the settings of the main variant of the package, the spacing before and after the icons can be set using the options \macro{before sep} and \macro{after sep}

\RenewCommandCopy\die\liiidie
\begin{codeexample}
Roll\die{eightside}{}a die!
