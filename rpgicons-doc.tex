...
\def\rpgiconsfileversion{2.9.0}
\def\rpgiconsfiledate{30 April 2026}
...
%\changes{v2.7.1}{2026/03/31}{Minor fix.}
%\changes{v2.7.2}{2026/04/10}{Bug fix.}
\changes{v2.8.0}{2026/04/15}{Added movement shapes.}
\changes{v2.9.0}{2026/04/30}{Added line cap and line join options for L3 variant.}
...
To install the package, copy the relevant package files \macro{rpgicons.sty}, \macro{rpgicons-l3.sty} and \macro{rpgicons-pgf.sty} as well as \macro{rpgicons.lua} into the working directory or into the \macro{texmf} directory. You may want to use a TeX package manager for this. After the package has been installed, the \macro{rpgicons} package is loaded by calling \macro{\usepackage{rpgicons}} in the preamble of the document.

The file \macro{rpgicons.sty} acts as a wrapper that loads either the file \macro{rpgicons-l3.sty} or \macro{rpgicons-pgf.sty} depending on the variant of the package that is used. Therefore, if only one of the variants should be used, technically speaking it is sufficient to only install on of these two files and load the package using \macro{\usepackage{rpgicons-l3}} or \macro{\usepackage{rpgicons-pgf}} respectively. Note, however, that the file \macro{rpgicons.lua} needs to be installed if the L3 variant is used with LuaLaTeX.
...
|l3|
\end{macrodef}
The L3 variant of the package is loaded by default. To load it explicitly, the package can be loaded using the option \macro{l3}.

\begin{macrodef}
|pgf|
\end{macrodef}
To load the PGF variant of the package the package needs to be loaded with the option \macro{pgf}.

\subsection{Dependencies}

The L3 variant of the package loads the \macro{l3draw} package. When compiled with LuaLaTeX, the file \macro{rpgicons.lua} needs to be installed.
...
\subsection[Command \textbackslash movement]{Command \titlemacro*{movement}}

\begin{macrodef}
|\movement|[<style>]{<shape>}[<options>]{<integer>}
\end{macrodef}
The command \macro{\movement} typesets an icon depicting a direction of movement.

The command takes two mandatory arguments, the first of which describes the shape (see the lists below) and the second taking an integer that is placed in front of the shape. For example, \macro{\movement{forward}{5}} results in \movement{forward}{5}. If tagging is enabled, the default replacement text of this command is ``5 forward''.

The command also takes two optional arguments, the second of which can take additional options to style the icon or to select different variants of the icon shape. The options affect the shape, but not the integer when it is typeset before the icon. The usable options differ depending on the package variant. See the relevant sections below.

The first optional argument can take the value \macro{normal} or \macro{large}, \macro{normal} being the default value. With \macro{large} given as argument, the icon is drawn larger and the integer is typeset inside the shape. As an example, \macro{\movement[large]{forward}{5}} results in \movement[large]{forward}{5}.

\begin{rpgiconsiconlist}
\titlemacro*{movement}
    & \movement{upward}{} & \macro{upward} \\
    & \movement{downward}{} & \macro{downward} \\
    & \movement{leftward}{} & \macro{leftward} \\
    & \movement{rightward}{} & \macro{rightward} \\
    & \movement{forward}{} & \macro{forward} \\
    & \movement{backward}{} & \macro{backward} \\
\end{rpgiconsiconlist}
...
|miter limit|={<float>}
\end{macrodef}
The option \macro{line width} sets the line width for strokes. The default line width is the default line width of Ti\emph{k}Z or the \macro{l3draw} package respectively which in both cases is 0.4\,pt.

The option \macro{line cap} takes as value one of \macro{butt}, \macro{rectangle} or \macro{round} and sets the style of stroke terminals to butt caps, rectangle caps (projecting square caps) or round caps respectively. Per default, round caps are set.

The option \macro{line join} takes as value one of \macro{bevel}, \macro{miter} or \macro{round} and sets the style of stroke joins to bevel joins, miter joins or round joins respectively. Per default, round line joins are set.

The option \macro{miter limit} takes as value floating point number and sets the miter factor of lines joined as a miter. The default factor is 10.
...
The \macro{scale inner} option can be used to change the scaling of the icon placed inside a frame when using the \macro{\ability}, \macro{\saving}, \macro{\spellschool}, \macro{\damage}, \macro{\class} and \macro{\alignment} macros. The default value is 0.675.
...
|\rpgiconsdirection|[<style>]{<shape>}[<options>]{<integer>}
\end{macrodef}
...
!rpg icons/!|every direction|/.style={<options>}
...
\end{macrodef}
All icons share the Ti\emph{k}Z style \macro{rpg icons} that is empty per default but can be used to style all icons at once. For example, if \macro{\tikzset{rpg icons/.append style={draw=red}}} is placed at the beginning of the document, all icons will be drawn in red. Per default, the icons are drawn in the color of the surrounding text. Per default, the Ti\emph{k}Z keys \macro{line cap} and \macro{line join} are set to \macro{round}.
...
\macro{\provideprotectedrpgicon{die}[large]{eightside}[blue, thick]{mybox}}, for example, stores the icon of an eight-sided die with the relevant style and Ti\emph{k}Z options in a new box named \macro{mybox}. Note that no integer can be added to the macros \macro{die}, \macro{currency} or \macro{direction} in this context.
...
The L3 variant of the package provides the following token variables and commands for defining user functions by package authors.

\begin{macrodef}
|\rpgicons_draw_shape_<<shape>>:|
\end{macrodef}
These commands contain the definitions of the shapes of the icons. If the shape name contains a space, this is substituted by an underscore in the command name. For example, the command \macro{\rpgicons_draw_shape_charisma:} contains the definition of the charisma shape, the command \macro{\rpgicons_draw_shape_psion_i:} contains the definition of variant one of the psion shape (\macro{psion i}).

The definition of a shape consists of a row of commands to describe one or more paths. The definition should be a valid code snippet that can be placed between the commands \macro{\draw_begin:} and \macro{\draw_path_use:n { stroke } \draw_end:}. The shape \macro{lawful neutral}, for example, has the following definition:

\begin{codeexample}
\cs_new_protected:Npn \rpgicons_draw_shape_lawful_neutral: {
...
\begin{macrodef}
|\rpgicons_draw_shape_sixside_pips:n| {<integer>}
\end{macrodef}
The command \macro{\rpgicons_draw_shape_sixside_pips:n} takes as argument an integer from one to nine and outputs the definition of paths representing the relevant number and arrangement of pips inside the shape of a sixside die.

\begin{macrodef}
|\rpgicons_draw_frame_ability:|
|\rpgicons_draw_frame_saving:|
|\rpgicons_draw_frame_spellschool:|
|\rpgicons_draw_frame_damage:|
|\rpgicons_draw_frame_class:|
|\rpgicons_draw_frame_alignment:|
\end{macrodef}
These commands contain the definitions of the relevant frames that are used by the relevant icon commands.

\begin{macrodef}
|\l_rpgicons_icon_before_sep_dim|
|\l_rpgicons_icon_after_sep_dim|
|\l_rpgicons_icon_baseline_dim|
|\l_rpgicons_icon_linewidth_dim|
\end{macrodef}

\begin{macrodef}
|\l_rpgicons_icon_frame_tl|
|\l_rpgicons_icon_color_stroke_tl|
|\l_rpgicons_icon_color_fill_tl|
|\l_rpgicons_icon_color_text_tl|
|\l_rpgicons_icon_color_background_tl|
\end{macrodef}

\begin{macrodef}
|\l_rpgicons_icon_variant_int|
\end{macrodef}

\begin{macrodef}
|\l_rpgicons_icon_opacity_stroke_fp|
|\l_rpgicons_icon_opacity_fill_fp|
|\l_rpgicons_icon_opacity_text_fp|
|\l_rpgicons_icon_opacity_background_fp|
|\l_rpgicons_icon_transform_scale_fp|
|\l_rpgicons_icon_transform_scale_inner_fp|
...
\end{macrodef}
These token lists contain the values set by the ueer via the relevant options. The floating point token lists \macro{\l_rpgicons_icon_opacity_stroke_fp}, \macro{\l_rpgicons_icon_opacity_fill_fp}, \newline \macro{\l_rpgicons_icon_opacity_text_fp}, \macro{\l_rpgicons_icon_opacity_background_fp}, \newline as well as \macro{\l_rpgicons_icon_transform_scale_fp} and \newline \macro{\l_rpgicons_icon_transform_scale_inner_fp} are initialised with the value 1.0.

\begin{macrodef}
|\l_rpgicons_icon_precompose_ref_str|
|\l_rpgicons_precompose_output_str|
\end{macrodef}
The token list \macro{\l_rpgicons_icon_precompose_ref_str} contain the values set by the ueer via the option \macro{precompose}.

The token list \macro{\l_rpgicons_precompose_output_str} contains the value set by the user via the option \macro{precompose output}. Its default value is \macro{\c_sys_jobname_str _rpgicons} which is the name of the current file with the suffix \macro{_rpgicons} added.

\begin{macrodef}
|\rpgicons_shape_set_eq:nn| {<shape>} {<shape>}
\end{macrodef}
The command \macro{\rpgicons_shape_set_eq:nn} sets the definition of the shape given in the first argument equal to that given in the second argument. If either shape name contains spaces, these are replaced by underscores. The relevant internal command is then set to contain the relevant shape definition. For example, \macro{\rpgicons_shape_set_eq:nn {foo} {charisma}} will create the command \macro{\rpgicons_draw_shape_foo:} as copy of \macro{\rpgicons_draw_shape_charisma:}. If the shape given in the first argument already exits, its definition will be replaced.
