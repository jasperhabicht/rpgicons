...
\end{macrodef}
The command \macro{\die} typesets an icon to depict a die with a certain number of sides.\footnote{The code for the dice shapes is based on the \href{https://tex.stackexchange.com/a/144474/47927}{idea of John Kormylo}.} Additionally, icons exist for a two-sided die (which would be equivalent to a coin) and for a hundred-sided die (which typically comes in the shape of a sphere). There is also a special icon for a fudge die.
