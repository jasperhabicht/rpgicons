...
\def\rpgiconsfileversion{2.6.0}
\def\rpgiconsfiledate{23 February 2026}
...
\changes{v2.6.0}{2026/02/23}{Added functionality to precompose icons in L3 variant. Update accessibility support.}
...
\titlemacro*{ability}
    ...
    & \ability{armour} & \macro{armour} or \macro{armor} \\
...
\titlemacro*{saving}
    ...
    & \saving{armour} & \macro{armour} or \macro{armor} \\
...
\titlemacro*{currency}
    ...
    & \currency{jewellery}[variant=1]{} & \macro{jewellery} or \macro{jewelry} & 1 \\
\end{rpgiconsiconlist}
...
It is possible to change the replacement texts for specific icon types. For example, setting the key \macro{every saving={every charisma={actualtext={charisma saving}}}} will set the replacement text for every instance of \macro{\saving{charisma}} to \macro{charisma saving}.
...
Due to the fact that styles such as colors and transformations have already been applied when the external PDF was generated, they won't have any effect when used together with the option \macro{precompose}.
